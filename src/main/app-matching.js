...
function rankAppCandidates(query, apps = []) {
  ...
  for (const app of apps) {
    ...
    else if (normalized && lower.includes(normalized) && normalized.length >= 3) score = 700;
    else if (normalized && normalized.includes(lower) && lower.length >= 4) score = 650;
    else if (normalized) {
...
function findInstalledAppSmart(appName, discoverInstalledApps) {
  const query = String(appName || '').trim();
  if (!query) return { match: null, suggestions: [], query: '' };
  const apps = discoverInstalledApps();
  const ranked = rankAppCandidates(query, apps);
  if (!ranked.length) {
    return { match: null, suggestions: [], query: normalizeAppQuery(query) };
  }
  const best = ranked[0];
  const second = ranked[1];
  const ambiguous = Boolean(second && (best.score - second.score) < 80 && second.score > 400);
  return {
    match: best.score >= 400 ? best.app : null,
    suggestions: ranked.slice(0, 5).map(item => item.name),
    ambiguous,
    query: normalizeAppQuery(query),
    alias: resolveAlias(query)
  };
}
